...
- **Desaturation post-FX:** A custom `DesaturationPipeline` (`src/systems/desaturationPipeline.ts`) is attached to the main camera in `GameScene.attachDesaturationPipeline`. It samples the LightingSystem's RenderTexture as a second sampler (texture unit 1; bound directly via `gl.activeTexture`/`gl.bindTexture` to avoid Phaser's PostFX wrapper API drift across versions). Per-fragment: maps the camera framebuffer UV into world UV using `cam.worldView` (passed each frame via `setFrameState`), samples the lighting mask alpha at that world UV, and mixes the original color toward luminance-weighted greyscale by `mask.alpha * LIGHTING_CONFIG.desaturationStrength`. `cam.worldView` is the canonical "what's visible in world coords" — re-deriving from scroll/zoom/screen-size produced wrong results across viewports. `uFlipY` uniform handles Phaser's bottom-up framebuffer Y convention. With `LIGHTING_CONFIG.enabled = false` OR no lighting RT bound, the shader passes the framebuffer through unchanged (`uEnabled` short-circuit). The UI camera has no post-FX so dialogue/joystick/HUD render in original colors regardless of fog state. Thoughts (depth 8, main camera) use white-on-dark text; the white pixels are unaffected by the luminance-weighted greyscale (R=G=B), so thought bubbles remain readable through fog without a separate camera split.
