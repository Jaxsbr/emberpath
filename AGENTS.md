...
| `data/areas/types.ts` | All shared types — AreaDefinition (includes required `tileset: string`, `props: PropDefinition[]`, and `decorations: DecorationDefinition[]`; `visual.floorColor/wallColor` retained for editor), ExitDefinition, NpcDefinition (required `sprite: string`, `wanderRadius: number`, `awarenessRadius: number`; `color` retained for editor map-overview), PropDefinition, DecorationDefinition (`{ col, row, spriteFrame }` — tile-snapped, no id, atlas-frame literal; consumed by GameScene.renderDecorations at depth 2), TriggerDefinition, DialogueScript, StorySceneDefinition |
