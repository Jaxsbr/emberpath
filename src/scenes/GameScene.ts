...
export class GameScene extends Phaser.Scene {
  ...
  private renderFallbackTiles(): void {
    ...
    for (let row = 0; row < this.area.mapRows; row++) {
      for (let col = 0; col < this.area.mapCols; col++) {
        // Cell colour: wall tone when the cell blocks (impassable terrain OR any
        // authored object sub-cell), floor tone otherwise. `cellBlocks` is the
        // single source of truth and aggregates the FB-23 sub-cell object map back
        // to cell granularity. Exit zones get their hope-gold tint from
        // renderExitOverlays at depth 0.5.
        const blocked = cellBlocks(col, row, this.passability);
  ...
  private renderObjects(): void {
    ...
    for (const inst of this.area.objects) {
      ...
      if (def.shadow) {
        ...
        // and NOT the footprint bottom out in the yard (#930 "purple"). The green
        // box spans the front wall from just under the eave line down to the wall
        // base, full front-wall width, and is rendered BENEATH the house sprite
        // (depth 0.6) so the walls draw over it and it reads as the house's own
        // grounding. Measured off his annotation onto the 4×4 sprite (rows
        // inst.row..inst.row+4): top ≈ row+bf.dy-0.9, height ≈ 1.1 tiles → the
        // band lands at world rows 14.1→15.2, centred on the sprite.
        const shCx = (inst.col + bf.dx + bf.w / 2) * TILE_SIZE;
        const shTopY = (inst.row + bf.dy - 0.9) * TILE_SIZE; // just under the eave
        const shH = TILE_SIZE * 1.1; // full front-wall band, down to the wall base
