...
const PLAYER_SHADOW_WIDTH = PLAYER_SIZE * 1.05;
const PLAYER_SHADOW_FOOT_OFFSET = 22;
const NPC_SHADOW_WIDTH = NPC_SIZE * 1.05;
const NPC_SHADOW_FOOT_OFFSET = 20;
const ENTITY_SHADOW_ALPHA = 0.26;
...
export class GameScene extends Phaser.Scene {
  ...
  private renderObjects(): void {
    ...
    for (const inst of this.area.objects) {
      ...
      // Believable ground-contact shadow (FB-3/FB-4/FB-21). Every shadow now goes
      // through the ONE canon (makeGroundShadow): a soft ellipse pooled on the
      // ground-contact line with a small south sliver, sized to the footprint. We
      // only compute each object's contact line + width here; the projection is
      // shared so buildings can't drift to their own shape/offset (the FB-8/FB-21
      // failure). Depth 0.6 keeps it above terrain but beneath the object and any
      // entity that walks over.
      const cf = def.collisionFootprint;
      ...
      let shW: number;
      if (bf) {
        // Buildings (cottage): anchor at the VISIBLE wall base — where the front
        // wall meets the ground in the art (~0.78 of sprite height) — NOT the
        // baseFootprint's bottom edge, which sits a tile lower out in the yard.
        // Anchoring to the footprint bottom was the FB-8/FB-21 bug: the shadow
        // rendered down in the grass ("purple is wrong"). The canon then tucks it
        // under the walls with a small south sliver, matching Pip's own shadow.
        shCx = (inst.col + fp.w / 2) * TILE_SIZE;
        contactY = (inst.row + fp.h * 0.78) * TILE_SIZE;
        shW = fp.w * TILE_SIZE * 0.62; // ~the visible wall width, centred on the house
      } else if (cf) {
        ...
        contactY = (inst.row + cf.dy + cf.h) * TILE_SIZE;
        ...
        shCx = (inst.col + fp.w / 2) * TILE_SIZE;
        contactY = (inst.row + fp.h) * TILE_SIZE - TILE_SIZE * 0.2;
        shW = fp.w * TILE_SIZE * 0.5;
      }
      const shadow = this.makeGroundShadow(shCx, contactY, shW, 0.26);
      shadow.setDepth(0.6);
