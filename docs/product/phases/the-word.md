...
Status: draft — awaiting Jaco sign-off on the allegory/gospel substance (see "Decisions for Jaco" below)
...
### Decisions for Jaco (gospel / allegory substance — his reserved domain, not mine)

These are the parts I will **not** finalize without your sign-off, per our boundary
(vision/allegory/gospel are yours). Everything else in this spec is mechanics/feel I
can build:

1. **Who gives the Word.** Two clean options, both reuse the keeper-spawn idiom:
   (a) **the Keeper returns** (the heron — Holy Spirit allegory; the master-prd note
   suggests "probably a return visit from the Keeper, or a second NPC functioning as a
   teacher"); or (b) **a new teacher NPC** (a steward/elder who keeps the Word). My
   lean is (a) — it needs no new art and ties grace→Word to the same hand. Your call.
2. **What the inscribed stones say.** The remembered lines are scripture-*allegory*
   content (gospel substance). I have drafted **placeholder** kid-level lines below
   (marked ⟨DRAFT — Jaco to approve/replace⟩); the final words are yours to author or
   approve. I will not ship invented scripture-substance.
3. **How "remember" frames faith.** That the Word is *received and kept* (not earned,
   not a buff you grind) is the doctrine under the allegory. I have kept the mechanic
   gift-shaped; confirm the framing reads right to you.
...
- Briar Wilds shipped with `EmberWarmthSystem` drain/quiet zones (the trial this phase
  matures).
- **Jaco sign-off on Decisions 1–3** before the content (giver, stone words, framing) is
  authored. Mechanics (US-W1, US-W3 plumbing, US-W4 engine, US-W5 harness) can be built
  against placeholder text in parallel.
