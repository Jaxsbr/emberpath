...
- [x] `package.json` lists `phaser` (^3.x), `typescript`, and `vite` as dependencies [US-01] [Shipped]
- [x] `npm run dev` starts a Vite dev server that renders a Phaser game canvas in the browser [US-01] [Shipped]
- [x] `npm run build` produces a `dist/` directory containing `index.html` and bundled JS [US-01] [Shipped]
- [x] `tsconfig.json` has `"strict": true` [US-01] [Shipped]
- [x] HTML page contains an explicit `<div id="game-container">` element; Phaser game config sets `parent: 'game-container'` [US-01] [Shipped]
- [x] Phaser game config sets `scale.mode` to `Phaser.Scale.FIT` and `scale.autoCenter` to `Phaser.Scale.CENTER_BOTH` [US-01] [Shipped]
- [x] A tile map data module exports a 2D array of tile types (minimum: floor = 0, wall = 1) [US-02] [Shipped]
- [x] `TILE_SIZE`, `MAP_COLS`, and `MAP_ROWS` are defined as named constants (not magic numbers) [US-02] [Shipped]
- [x] GameScene renders the tile map with visually distinct floor and wall tiles — floor reads as "walkable space" (lighter color), wall reads as "solid barrier" (darker/contrasting color) [US-02] [Shipped]
- [x] Collision detection prevents the player entity from overlapping wall tile positions [US-02] [Shipped]
- [x] Player entity renders as a colored rectangle visually distinct from tile map elements — reads as "the character you control" (different color from both floor and wall tiles) [US-03] [Shipped]
- [x] Pressing W/A/S/D moves the player up/left/down/right respectively [US-03] [Shipped]
- [x] Virtual joystick appears on touch input and moves the player in the drag direction [US-03] [Shipped]
- [x] Player movement is frame-based smooth movement (position updates per frame), not tile-snapping [US-03] [Shipped]
- [x] Player cannot move into or through wall tiles — collision stops movement at tile boundary [US-03] [Shipped]
- [x] Movement speed is defined as a named constant (e.g., `PLAYER_SPEED`) [US-03] [Shipped]
- [x] Camera follows the player entity during movement [US-04] [Shipped]
- [x] Camera bounds are set to the tile map pixel dimensions — no empty/black space visible beyond map edges [US-04] [Shipped]
- [x] Camera behavior is consistent across mobile (375x667) and desktop (1280x720) viewport sizes [US-04] [Shipped]
- [x] TitleScene is the first scene loaded by the Phaser game [US-05] [Shipped]
- [x] TitleScene displays "Emberpath" text centered horizontally on screen [US-05] [Shipped]
- [x] TitleScene contains a "Start" interactive element that responds to `pointerdown` event [US-05] [Shipped]
- [x] Clicking/tapping Start transitions to GameScene — scene switch occurs and GameScene renders the tile map [US-05] [Shipped]
- [x] `src/` directory has separate modules for input handling, movement/collision, and scene management — establishing the systems-based entity architecture from the master PRD [phase] [Shipped]
- [x] AGENTS.md includes an explicit depth map defining rendering order for visual layers (e.g., tiles: 0, entities: 5, UI: 100) [phase] [Shipped]
- [x] AGENTS.md reflects new modules, directories, file ownership, and behavior rules introduced in this phase [phase] [Shipped]
- [x] README.md documents how to run the project locally (`npm install`, `npm run dev`) and lists controls (WASD on desktop, virtual joystick on mobile) [phase] [Shipped]
- [x] Verify command configured in progress.yaml: `npx tsc --noEmit && npm run build` [phase] [Shipped]
