...
| [implementation/prefetch-erases-the-wait-mid-scene-loader-start](implementation/prefetch-erases-the-wait-mid-scene-loader-start.md) | You have a per-area/per-route lazy loader and want most transitions to show NO loader; or adding a progress bar / waiting text that should feel shorter; or running Phaser's loader mid-scene | Warm the next area's bundle **during gameplay** — idle prefetch when the player lingers near a usable exit (throttled `this.load.start()`, `isLoading()`-guarded, reuses `queueAreaAssets`, add-before-queue) → 0ms loader on the common path. Cold case: **known-progress fill + rotating whispers** (tear down the `progress` listener AND whisper timer on `complete`). Prefetch self-merges; visuals are GATE-2. |
