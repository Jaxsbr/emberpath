...
- [x] Pressing W plays `fox-pip-walk-north`; D plays `fox-pip-walk-east`; S plays `fox-pip-walk-south`; A plays `fox-pip-walk-west` [US-44]
- [x] Diagonal input (e.g., W+D) zeroes the lesser-magnitude axis — movement is single-axis only; a code comment marks diagonal suppression as temporary until diagonal sprites arrive [US-44]
- [x] Equal-magnitude diagonal input (keyboard W+D) maintains current facing direction rather than flip-flopping [US-44]
- [x] Walk animation frameRate is 8 [US-44]
- [x] Movement speed during walk is PLAYER_SPEED (160 px/s) [US-44]
- [x] After holding continuous movement input for ≥2s, animation switches from `fox-pip-walk-{dir}` to `fox-pip-run-{dir}` [US-45]
- [x] `RUN_MULTIPLIER` constant exists in `src/maps/constants.ts`; run speed = PLAYER_SPEED × RUN_MULTIPLIER [US-45]
- [x] `RUN_THRESHOLD_MS` constant exists in `src/maps/constants.ts`; walk-to-run timer uses this value [US-45]
